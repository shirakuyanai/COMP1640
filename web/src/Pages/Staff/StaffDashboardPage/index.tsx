...
function StaffDashboardPage() {
	...
	useEffect(() => {
		if (socket) {
			socket.on('dashboardUpdate', (update: { type: string, data: any }) => {
	...
	const handleSaveClassName = async (classId: string) => {
		try {
			...
			const response = await fetch(`/api/classes/${classId}`, {
				...
				body: JSON.stringify({ className: editingClassName })
			});
			...
			if (response.ok) {
				...
				const updatedClasses = rawData.classes.map(c => 
					c.id === classId 
						? {...c, className: editingClassName} 
						: c
				);
				...
				// Remove references to recentActivities
				// Update dashboard data with new class info without activities
				setDashboardData({
					...dashboardData,
					// Updated class data
					totalClasses: updatedClasses.length,
