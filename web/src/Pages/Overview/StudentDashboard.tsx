...
function StudentDashboard() {
	...
	const calculateProgress = (
		...
		try {
			...
			if (!isValid(start) || !isValid(end)) {
				console.error('Invalid dates for progress calculation:', {
					startDate,
					endDate,
				})
				return 0
			...
			try {
				...
				for (const classItem of classes) {
					try {
						...
						console.error(
							`Error fetching meetings for class ${classItem.id}:`,
							error,
						)
					}
				}
				...
				setStats({
					...
					attendedMeetings: allMeetings.filter(
						(m: MeetingType) => m.studentAttended === 1,
					).length,
	...
	const StatCard = ({ icon, title, value, description, color }: any) => (
		<Card className='border-none shadow-md hover:shadow-lg transition-shadow duration-200'>
			<CardContent className='flex items-center p-4 md:p-6'>
				<div className={`${color} p-3 md:p-4 rounded-xl shadow-sm`}>{icon}</div>
				<div className='ml-3 md:ml-4'>
					<p className='text-xs md:text-sm font-medium text-slate-500'>
						{title}
					</p>
					<h4 className='text-lg md:text-2xl font-bold text-slate-800'>
						{value}
					</h4>
	...
	return (
		<div className='p-3 md:p-8 space-y-4 md:space-y-8 bg-slate-50 min-h-screen'>
			...
			<div className='bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-100'>
				<>
					...
					<p className='text-sm md:text-base text-slate-500'>
						Here's a summary of your interactions with your personal tutor
					</p>
				</>
			</div>
			...
			<div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6'>
				<StatCard
					icon={
						<FaChalkboardTeacher className='text-indigo-600 h-5 w-5 md:h-6 md:w-6' />
					}
					title='Total Classes'
				...
				<StatCard
					icon={
						<FaCalendarDays className='text-blue-600 h-5 w-5 md:h-6 md:w-6' />
					}
				...
				<StatCard
					icon={
						<FaCalendarCheck className='text-green-600 h-5 w-5 md:h-6 md:w-6' />
					}
					title='Attended'
				...
				<StatCard
					icon={
						<FaGraduationCap className='text-amber-600 h-5 w-5 md:h-6 md:w-6' />
					}
					title='Upcoming'
				...
				<StatCard
					icon={
						<FaComments className='text-purple-600 h-5 w-5 md:h-6 md:w-6' />
					}
					title='Completed'
					...
					<div className='space-y-3 md:space-y-4'>
						{classes.length === 0 ? (
							...
							classes.map((classItem) => (
								<div
									key={classItem.id}
									className='p-4 md:p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 hover:bg-blue-50 transition-colors'
								>
			...
			<Card className='border-none shadow-md'>
				...
				<CardContent className='bg-white rounded-b-xl p-3 md:p-6'>
					<div className='space-y-3 md:space-y-4'>
						{meetings.length === 0 ? (
							<p className='text-slate-500 text-center py-4 md:py-8'>
								No meetings found
							</p>
						) : (
							meetings.slice(0, 5).map((meeting) => (
								<div
									...
									className='p-4 md:p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 transition-colors'
								>
									<div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-0'>
										<div>
											<h3 className='font-semibold text-base md:text-lg text-slate-800'>
										...
										<div className='text-right'>
											...
											<p
												className={`text-xs md:text-sm font-medium mt-2 px-2 md:px-3 py-1 md:py-1.5 rounded-lg ${
