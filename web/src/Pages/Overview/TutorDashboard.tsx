  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
        <h1 className="text-3xl font-bold text-slate-800 mb-1">Welcome, {currentUser?.username}!</h1>
        <p className="text-slate-500">Here's a summary of your tutoring activities</p>
      </div>

      {/* Statistics */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        <StatCard
          icon={<FaChalkboardTeacher className="text-indigo-600 h-6 w-6" />}
          title="Total Classes"
        ...
        />
        <StatCard
          icon={<FaCalendarCheck className="text-amber-600 h-6 w-6" />}
          title="Upcoming"
        ...
        />
        <StatCard
          icon={<FaComments className="text-purple-600 h-6 w-6" />}
          title="Completed"
      ...
      <Card className="border-none shadow-md">
        <CardHeader className="border-b border-slate-100 bg-white rounded-t-xl flex flex-row items-center justify-between">
          <CardTitle className="text-slate-800">My Classes</CardTitle>
          <div className="flex gap-4">
            <Input
              placeholder="Search by class name..."
              value={classSearchTerm}
              onChange={(e) => setClassSearchTerm(e.target.value)}
              className="w-64 border-slate-200 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
            />
          </div>
        </CardHeader>
        <CardContent className="bg-white rounded-b-xl p-6">
          <div className="space-y-4">
            {filteredClasses.map((classItem) => (
              <div key={classItem.id} className="p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 hover:bg-blue-50 transition-colors">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-semibold text-lg text-slate-800">{classItem.className}</h3>
                  </div>
                  <div className="text-right text-sm text-slate-500 bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-100">
      ...
      <Card className="border-none shadow-md">
        <CardHeader className="border-b border-slate-100 bg-white rounded-t-xl flex flex-row items-center justify-between">
          <CardTitle className="text-slate-800">My Students</CardTitle>
          <div className="flex gap-4">
            <Input
        ...
        <CardContent className="bg-white rounded-b-xl p-6">
          <div className="space-y-4">
            {filteredStudents.map((student) => (
              <div key={student.username} className="p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 hover:bg-blue-50 transition-colors">
                <div>
                ...
                <div className="mt-4">
                  <p className="text-sm text-slate-500 mb-2 font-medium">Classes:</p>
                  <div className="flex flex-wrap gap-2">
                    {student.classes.map((classItem: ClassType) => (
                      <span key={classItem.id} className="px-3 py-1.5 bg-white text-slate-700 rounded-md text-sm border border-slate-200 shadow-sm">
                        {classItem.className}
                      </span>
                    ))}
                  </div>
      ...
      <Card className="border-none shadow-md">
        <CardHeader className="border-b border-slate-100 bg-white rounded-t-xl">
          <CardTitle className="text-slate-800">Recent Meetings</CardTitle>
        </CardHeader>
        <CardContent className="bg-white rounded-b-xl p-6">
          <div className="space-y-4">
            {meetings.length === 0 ? (
              <p className="text-slate-500 text-center py-8">No meetings found</p>
            ) : (
              meetings.slice(0, 5).map((meeting) => (
                <div key={meeting.meetingId} className="p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 transition-colors">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-semibold text-slate-800">{meeting.className}</h3>
                      <p className="text-sm text-slate-500 mt-1">
                        Type: <span className="capitalize">{meeting.meetingType}</span>
                      </p>
                      {meeting.meetingNotes && (
                        <p className="text-sm text-slate-600 mt-3 bg-white p-3 rounded-lg border border-slate-100">
                          {meeting.meetingNotes}
                        </p>
                      )}
                    </div>
                    <div className="text-right">
                      <p className="text-sm bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-100">
                        {meeting.meetingDate ? formatDateTime(meeting.meetingDate) : 'Date not set'}
                      </p>
                      <p className={`text-sm font-medium mt-2 px-3 py-1.5 rounded-lg ${
