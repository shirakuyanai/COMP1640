	...
	return (
		<div className='p-8 space-y-8 bg-slate-50 min-h-screen'>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
				<StatCard
					icon={<FaCalendar className="text-indigo-600 h-6 w-6" />}
				...
				/>
				<StatCard
					icon={<FaClock className="text-green-600 h-6 w-6" />}
					title="In Progress"
			...
			<Card className="border-none shadow-md">
				<CardHeader className="border-b border-slate-100 bg-white rounded-t-xl">
					<CardTitle className="text-slate-800 text-lg">Class Status Legend</CardTitle>
				</CardHeader>
				<CardContent className="bg-white rounded-b-xl p-6">
					<div className="flex flex-col sm:flex-row gap-6">
						{Object.entries(STATUS_COLORS).map(([status, config]) => (
							<div key={status} className={`flex items-center gap-3 px-4 py-3 rounded-lg ${config.lightBg} ${config.border} border`}>
								<div className={`w-4 h-4 rounded-full ${config.bg}`}></div>
								<span className="text-sm text-slate-700">{config.text}</span>
							</div>
						))}
					</div>
				</CardContent>
			...
			{/* Classes Grid */}
			<div>
				<h2 className='text-2xl font-bold mb-6 text-slate-800 pl-1'>
				...
				{loading ? (
					...
					<Card className="border-none shadow-md bg-white p-12 text-center">
						<div className="flex flex-col items-center justify-center">
							...
							<CardTitle className="text-slate-700 mb-2">No Classes Found</CardTitle>
...
const ClassCard = ({ 
	...
	return (
		<Card className="border-none shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden group">
			<CardHeader className={`bg-gradient-to-r ${statusGradient} p-3 group-hover:opacity-90 transition-opacity`} />
			<CardContent className="p-6 pt-5">
				<Link to={`/dashboard/classes/${id}`} className="block h-full">
					<h2 className='text-xl font-bold mb-2 text-slate-800 group-hover:text-blue-700 transition-colors'>{className}</h2>
					...
					<div className='space-y-4'>
						...
						<div className='space-y-3'>
							...
							{/* Status indicator */}
							<div className={`flex items-center gap-2 mt-3 p-2 rounded-lg ${statusBg} ${statusBorder} border`}>
								<div className={`w-3 h-3 rounded-full ${STATUS_COLORS[status].bg}`}></div>
								<span className="text-sm capitalize font-medium text-slate-700">{status.replace('-', ' ')}</span>
							</div>
						</div>
					</div>
				</Link>
